...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    $del_stmt = $conn->prepare("DELETE FROM comment WHERE target_type='standpoint' AND target_id=?");
...
$total = countWhere($conn, 'standpoint', $where, $count_types, $params);
$offset = ($current_page - 1) * $page_size;
$list_sql = "SELECT id, title, cover, created_at FROM standpoint WHERE $where ORDER BY created_at DESC, id DESC LIMIT $offset, $page_size";
