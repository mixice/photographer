...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        $content = moveTmpFiles($content, 'photography');
        cleanUnusedFiles($row['cover'] ?? '', $cover, $row['content'] ?? '', $content);
        $stmt = $conn->prepare("UPDATE photography SET title=?, cover=?, content=?, comment_enabled=? WHERE id=?");
        ...
        $content = moveTmpFiles($content, 'photography');
        $stmt = $conn->prepare("INSERT INTO photography (title, cover, content, comment_enabled, created_at) VALUES (?, ?, ?, ?, ?)");
